    ...
    <div className="w-full max-w-3xl mx-auto space-y-8">
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right text-primary">Power (MW)</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {standardAmps.map((A) => {
                ...
                return (
                  <TableRow key={A}>
                    <TableCell className="font-medium">{A < 0.01 ? "4.16 mA (500mW)" : `${A} A`}</TableCell>
                    <TableCell>{voltage} V</TableCell>
                    {type !== "DC" && <TableCell>{pf}</TableCell>}
                    <TableCell className="text-right font-mono font-bold">{MW < 0.001 ? (W / 1000).toFixed(2) + " kW" : MW.toFixed(4) + " MW"}</TableCell>
                  </TableRow>
                );
              })}
            </TableBody>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-2">
            <TableIcon className="size-5 text-primary" />
            <CardTitle>Industrial & High Voltage Benchmarks (11kV, 33kV, 4000A)</CardTitle>
          </div>
        </CardHeader>
        <CardContent>
          <Table>
              ...
              {industrialBenchmarks.map((item, idx) => {
                ...
                return (
                  <TableRow key={idx}>
                    ...
                    <TableCell className="text-right font-mono font-bold text-primary">{MW.toFixed(2)} MW</TableCell>
                  </TableRow>
                );
              })}
            </TableBody>
          </Table>
          <p className="text-[10px] text-muted-foreground mt-4 italic">* Industrial calculations assume 3-Phase L-L system with PF={pf}.</p>
        </CardContent>
      </Card>
    </div>
  );
