...
export function AmpsToWattsCalculator() {
  ...
  const [type, setType] = useState<CalculationType>("DC");
  ...
  return (
    <div className="w-full max-w-3xl mx-auto space-y-8 p-4 md:p-8">
      <div className="space-y-6">
        ...
        <Tabs
          className="w-full"
          onValueChange={(v) => setType(v as CalculationType)}
          value={type}
        >
          <TabsList className="w-full">
            {TYPES.map((t) => (
              <TabsTrigger className="flex-1" key={t.value} value={t.value}>
                {t.label}
              </TabsTrigger>
            ))}
          </TabsList>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-start gap-2 p-3 bg-amber-50 dark:bg-amber-950/20 rounded-lg border border-amber-100 dark:border-amber-900/30">
                  ...
                  <p className="text-xs text-amber-800 dark:text-amber-400 leading-relaxed">
                    AC systems require a power factor. Resistive loads are
                    usually 1.0, while inductive loads (like motors) are less
                    than 1.0.
                  </p>
                </div>
              </div>
