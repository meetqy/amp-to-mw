    ...
    <Card className="w-full max-w-3xl mx-auto">
      <CardHeader>
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">kV to MW Calculator</h1>
        </div>
        <CardDescription>Calculate Megawatts (MW) from Kilovolts (kV) and Amperes (A) for 3-Phase AC systems.</CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-8">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              <Input id="voltage" type="number" value={voltageKv} onChange={(e) => setVoltageKv(e.target.value)} placeholder="e.g. 33" />
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              <Input id="current" type="number" value={current} onChange={(e) => setCurrent(e.target.value)} placeholder="e.g. 100" />
          ...
          <div className="space-y-3">
            ...
            <Input id="pf" type="number" step="0.01" min="0" max="1" value={pf} onChange={(e) => setPf(e.target.value)} placeholder="0.8" />
          </div>
        </div>
        ...
        <div className="relative overflow-hidden bg-primary rounded-2xl p-8 text-primary-foreground shadow-lg shadow-primary/20 transition-all hover:scale-[1.01]">
          <div className="relative z-10 flex flex-col items-center justify-center">
            <p className="text-sm font-medium opacity-80 mb-2">Calculated Power (MW)</p>
            <div className="flex items-baseline gap-2">
              <span className="text-5xl font-black tracking-tight">{resultMw.toLocaleString(undefined, { maximumFractionDigits: 3 })}</span>
              <span className="text-2xl font-bold opacity-80 uppercase">MW</span>
            </div>
            <div className="mt-6 px-4 py-1.5 bg-white/10 rounded-full border border-white/20 backdrop-blur-sm">
              <code className="text-xs font-mono">Formula: P(MW) = (√3 × kV × A × PF) / 1000</code>
            </div>
          </div>
        ...
        <div className="flex items-start gap-2 p-3 bg-amber-50 dark:bg-amber-950/20 rounded-lg border border-amber-100 dark:border-amber-900/30">
          ...
          <p className="text-xs text-amber-800 dark:text-amber-400 leading-relaxed">
            <strong>Note:</strong> kV cannot be converted to MW directly. You must know the current (Amps) and Power Factor to calculate the actual power.
          </p>
        </div>
      </CardContent>
    </Card>
